...
class LogMelFilterBanks(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        stft_tensor = self.spectrogram(x)
        magnitude = torch.abs(stft_tensor)
        power_spectrum =magnitude ** self.power
        mel_spec = torch.matmul(self.mel_fbanks.T.to(power_spectrum.device), power_spectrum)
