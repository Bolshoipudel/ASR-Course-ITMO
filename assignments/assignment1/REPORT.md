...
В качестве базовой модели для экспериментов с групповыми свертками выбрана конфигурация **n_mels=80** (44 065 параметров, 99.39% test accuracy). Несмотря на то, что n_mels=20 и n_mels=40 показывают чуть лучшую точность, n_mels=80 обеспечивает наиболее детальное спектральное представление и является стандартным значением в ASR-системах.
